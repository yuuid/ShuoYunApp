    ...
    fetchData = () => {
        ...
        config.getUser().then(user => {
            ...
            if (user.userId) {
                ...
                let arrList = [];
                fetch(config.api.URI + '/activity/getArticles', {
                    method: 'POST',
                    headers: {
    ...
    renderContent = () => {
        ...
        return (
            <View style={{ flexDirection: 'column', minHeight: height }}>
                <Header
                    leftComponent={{ icon: 'chevron-left', color: '#979797', type: 'feather', onPress: () => { goBack(); } }}
                    centerComponent={{ text: detail.title, style: { color: '#333333', fontSize: 16 } }}
                    rightComponent={{ icon: 'star', color: isStar ? '#ff4d4d' : '#fecb37', type: 'feather', onPress: () => { this.setState({ isStar: !isStar }) } }}
                    backgroundColor={'#fff'}
                    style={{ height: 80 }}
                />
                <Image style={{ width, height: 200 }} resizeMode='cover' source={{ uri: detail.imgs || 'https://wx4.sinaimg.cn/mw690/4ca9b00fgy1g1fdftkw1xj20zk0k0myt.jpg' }} />

                <View style={styles.activityDetail}>
                    {content}
                </View>
                <Divider style={{ marginTop: 10 }} />
                <View style={{ paddingTop: 10, backgroundColor: '#fff', flexDirection: 'row', justifyContent: 'space-between' }}>
                    <View style={{ paddingLeft: 10 }}>
                        <Text style={{ fontSize: 14, marginLeft: 10, color: '#ff7040' }}>活动文章 {list.length}</Text>
                    </View>
                    <View style={{ paddingRight: 10 }}>
                        <Button title='投稿' size='sm' type='secondary' onPress={() => { navigate('ZhengWen', { activity: detail }) }} />
                    </View>
                </View>
