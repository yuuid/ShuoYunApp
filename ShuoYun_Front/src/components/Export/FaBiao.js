...
export default class FaBiao extends Component {
    ...
    handleType = () => {
        const sexOption = [
            { title: '文章', onPress: () => this.setState({ articleType: '文章', type: 1 }) },
            { title: '书评', onPress: () => this.setState({ articleType: '书评', type: 2 }) },
        ];
        ActionSheet.show(sexOption, cancelItem)
    }

    selectTime = () => {
        this.setState({ isShowstartDatePicker: true })
    }
    selectTime2 = () => {
        this.setState({ isShowendDatePicker: true })
    }

    showActivity = () => {
        // Toast.show('发新活动');
        this.setState({ isActivity: true, isArticle: false })
    }

    showArticle = () => {
        const { user } = this.state
        if (user.usertype == 1) { this.setState({ isActivity: false, isArticle: true }) }
    }

    renderContent = () => {
        const { imgList, type, articleType, user, start, end, isActivity, isArticle, isShowstartDatePicker, isShowendDatePicker } = this.state
        ...
        return (
            <View style={{ flexDirection: 'column', minHeight: height }}>
                ...
                />
                {isArticle && <View style={{ flexDirection: 'column', minHeight: height }}>
                    <View style={{ marginTop: 5 }}>
                        <ListRow
                            icon={<Icon name='tag' type='feather' color='#FF7040' />}
                            title={articleType}
                ...
                {isActivity && <View style={{ flexDirection: 'column', minHeight: height }}>
